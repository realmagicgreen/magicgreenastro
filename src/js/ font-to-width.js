...
;(function () {
  ...
  var FontToWidth = function (options) {
    ...
    var browserSupportsVarFonts =
      typeof document.createElement('span').style.fontVariationSettings ===
      'string'
    ...
    if (options.variableFont) {
      if (browserSupportsVarFonts) {
        ...
        options.axisMin = isNaN(parseFloat(options.variableFont.min))
          ? 1
          : options.variableFont.min
        options.axisMax = isNaN(parseFloat(options.variableFont.max))
          ? 1000
          : options.variableFont.max
      } else {
        console.log(
          'FontToWidth warning: variable mode specified, but this browser does not suport variable fonts'
        )
        if (!options.fonts) {
    ...
    options.minLetterSpace =
      typeof options.minLetterSpace === 'number'
        ? options.minLetterSpace
        : -0.04
    options.minFontSize =
      options.minFontSize || (ftw.mode == 'scale' ? 0.01 : 1.0)
    options.maxFontSize =
      options.maxFontSize || (ftw.mode == 'scale' ? 100 : 1.0)
  ...
  FontToWidth.prototype.measureFonts = function () {
    ...
    if (document.fonts && 'ready' in document.fonts) {
      ...
      ftw.options.fonts.forEach(function (font) {
        ffos.push(
          new FontFaceObserver(font.fontFamily.replace(/"/g, ''), {
  ...
  FontToWidth.prototype.updateWidths = function () {
    ...
    ftw.options.avgFontSize =
      (ftw.options.maxFontSize + ftw.options.minFontSize) / 2
    ...
    ftw.stillToDo.forEach(function (el) {
      el.removeClass('ftw_done ftw_final ftw_onemore')
    })

    var fvsRE = (window.re = new RegExp(
      '(,/s*)?[\'"]' + ftw.options.axis + '[\'"] [\\-\\d\\.]+',
      'g'
    ))
    function updateFVS(el, val) {
      el.style.fontVariationSettings = el.style.fontVariationSettings.replace(
        fvsRE,
        '"' + ftw.options.axis + '" ' + val
      )
    ...
    if (ftw.mode === 'variable') {
      ...
      ftw.allTheElements.forEach(function (el) {
        ...
        var info = {
          el: el,
          val: el.hasAttribute('data-axis-val')
            ? parseFloat(el.getAttribute('data-axis-val'))
            : (ftw.options.axisMin + ftw.options.axisMax) / 2,
      ...
      var doEverything = function (elements) {
        ...
        elements.forEach(function (info, i) {
          ...
          newFVS.push(
            currentStyle.length && currentStyle !== 'normal'
              ? currentStyle + ', ' + axisString
              : axisString
          )
        })
        ...
        while (anyLeft && tries--) {
          ...
          elements.forEach(function (info, i) {
            ...
            if (info.val < info.min + info.mille) {
              ...
              !isNaN(info.previous) &&
              Math.abs(info.val - info.previous) < info.mille
            ) {
              info.done = true
              --anyLeft
              return
    ...
    function adjustFontSizeAndLetterSpacing(cell, lasttime) {
      ...
      var newfontsize = fontsize,
        ...
        ratioToFit = fullwidth / textwidth
      ...
      if (
        letterspace >= ftw.options.minLetterSpace ||
        ...
        cell.style.letterSpacing =
          Math.max(letterspace, ftw.options.minLetterSpace) + 'em'
    ...
    function updateSingleWidth(cell) {
      ...
      if (ftw.mode === 'variable') {
        ...
        var axisMin = parseFloat(
          cell.getAttribute('data-axis-min') || ftp.options.axisMin
        )
        var axisMax = parseFloat(
          cell.getAttribute('data-axis-max') || ftp.options.axisMax
        )
      ...
      //for the widest font, we can max out the size
      if (
        (cell.getAttribute('data-biggest-font') || axisVal == axisMax) &&
        ratioToFit > ftw.options.maxFontSize
      ) {
        ratioToFit = ftw.options.maxFontSize
      }
  ...
  if (!HTMLElement.prototype.hasClass) {
    HTMLElement.prototype.hasClass = function (str) {
      ...
      words.forEach(function (word) {
        found =
          found && el.className.match(new RegExp('(^|\\s)' + word + '($|\\s)'))
      })
      return !!found
    }
  }
  ...
  function hyphenToCamel(hyphen) {
    switch (typeof hyphen) {
      ...
      case 'string':
        return hyphen.replace(/-([a-z])/g, function (x, letter) {
